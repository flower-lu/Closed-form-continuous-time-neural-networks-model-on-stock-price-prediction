...
class CfcCell(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        '''if isinstance(input_shape[0], tuple):：这里检查输入形状是否是嵌套元组。
        这种情况通常出现在使用多输入的情况下，其中输入可能是一个元组，每个元组中包含不同的输入。因此，我们需要检查第一个元素是否是元组。
        input_dim = input_shape[0][-1]：如果输入形状是嵌套元组，我们从最内层的元组中获取最后一个元素，即输入的维度。
        这是因为每个元组可能表示不同输入的形状，我们需要考虑最内层的元组。
        else:：如果输入形状不是嵌套元组，则直接从输入形状中获取最后一个维度作为输入的维度。
        '''
        ...
        # f 作为网络 s 型时间门的液体时间常数，self.time_a和self.time_b也是两个全连接层，它们通常用于处理与时间相关的特征
        else:
            self.ff1 = tf.keras.layers.Dense(
                self.units,
                lecun_tanh,
    ...
    def call(self, inputs, states, **kwargs):
        ...
        x = self.backbone(x)
        # 这段代码实现了模型的前向传播过程，其中输入数据与隐藏状态组合后，通过backbone模型进行处理，以产生模型的输出。
    ...
    def get_config(self):
        ...
        config.update({
            ...
            'no_gate': self._no_gate,
            'backbone': tf.keras.layers.serialize(self.backbone),
            'ff1': tf.keras.layers.serialize(self.ff1),
            'ff2': tf.keras.layers.serialize(self.ff2),
            'time_a': tf.keras.layers.serialize(self.time_a),
            'time_b': tf.keras.layers.serialize(self.time_b),
        })
        return config
