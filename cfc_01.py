...
def eval(config, index_arg, verbose=2):
    ...
    # Calculate price change between consecutive days
    price_diff = true_test[1:] - true_test[:-1]
    binary_labels = (price_diff > 0).astype(int)  # 1 if price increases, 0 otherwise

    # Make predictions on testing set
    predicted_prices = model.predict((x_test, x_date_test))

    # Denormalize predicted prices
    predicted_prices_denorm = predicted_prices * std_values[0] + mean_values[0]

    # Calculate price change for predicted prices
    predicted_price_diff = predicted_prices_denorm[1:] - predicted_prices_denorm[:-1]
    predicted_binary_labels = (predicted_price_diff > 0).astype(int)  # 1 if price increases, 0 otherwise

    # Evaluate model performance
    accuracy = accuracy_score(binary_labels, predicted_binary_labels)
    precision = precision_score(binary_labels, predicted_binary_labels)
    recall = recall_score(binary_labels, predicted_binary_labels)
    f1 = f1_score(binary_labels, predicted_binary_labels)
    conf_matrix = confusion_matrix(binary_labels, predicted_binary_labels)
